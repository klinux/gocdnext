...
func TestK8sAppSyncer_BuildsOperationPatch(t *testing.T) {
	...
	if err := s.syncApplication(context.Background(), target, "abc123"); err != nil {
...
func TestK8sAppSyncer_EmptyRevisionOmitsIt(t *testing.T) {
	...
	if err := s.syncApplication(context.Background(), target, ""); err != nil {
		t.Fatalf("syncApplication: %v", err)
	}
	var got map[string]any
...
func TestK8sAppSyncer_ValidatesTarget(t *testing.T) {
	p := &fakePatcher{resp: []byte(`{}`)}
	s := newK8sAppSyncer(p)
	if err := s.syncApplication(context.Background(), DeploymentTarget{ProjectID: uuid.New(), Cluster: "c"}, "r"); err == nil {
...
func (f *fakeSyncer) syncApplication(_ context.Context, _ DeploymentTarget, revision string) error {
	f.called = true
	f.gotRev = revision
	return f.gotError
}
...
func TestArgoProvider_Sync(t *testing.T) {
	t.Run("trigger mode actuates with the revision", func(t *testing.T) {
		fs := &fakeSyncer{}
