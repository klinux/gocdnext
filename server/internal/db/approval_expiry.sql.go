...
const expireApprovalRun = `-- name: ExpireApprovalRun :one
UPDATE runs
SET status = 'canceled',
...
type ExpireApprovalRunParams struct {
	ID           pgtype.UUID
	CancelReason *string
}

type ExpireApprovalRunRow struct {
	ID                pgtype.UUID
...
func (q *Queries) ExpireApprovalRun(ctx context.Context, arg ExpireApprovalRunParams) (ExpireApprovalRunRow, error) {
	row := q.db.QueryRow(ctx, expireApprovalRun, arg.ID, arg.CancelReason)
	var i ExpireApprovalRunRow
	err := row.Scan(&i.ID, &i.ServiceGeneration)
	return i, err
}

const listPendingApprovalGates = `-- name: ListPendingApprovalGates :many
...
WHERE j.approval_gate = true
  ...
  AND j.awaiting_since < $1::timestamptz
  AND (j.awaiting_since > $2::timestamptz
       OR (j.awaiting_since = $2::timestamptz
           AND j.id > $3::uuid))
...
type ListPendingApprovalGatesParams struct {
	OlderThan   pgtype.Timestamptz
	CursorSince pgtype.Timestamptz
	CursorID    pgtype.UUID
	PageSize    int32
}

type ListPendingApprovalGatesRow struct {
	ID            pgtype.UUID
	RunID         pgtype.UUID
	Name          string
...
func (q *Queries) ListPendingApprovalGates(ctx context.Context, arg ListPendingApprovalGatesParams) ([]ListPendingApprovalGatesRow, error) {
	rows, err := q.db.Query(ctx, listPendingApprovalGates,
		arg.OlderThan,
		arg.CursorSince,
		arg.CursorID,
		arg.PageSize,
	...
	defer rows.Close()
	items := []ListPendingApprovalGatesRow{}
	for rows.Next() {
		var i ListPendingApprovalGatesRow
		if err := rows.Scan(
			&i.ID,
			&i.RunID,
			&i.Name,
...
const markApprovalGateExpired = `-- name: MarkApprovalGateExpired :one
UPDATE job_runs
SET decision = 'expired', decided_at = NOW()
WHERE id = $1
  AND run_id = $2
  AND awaiting_since = $3
  AND approval_gate = true
  AND status = 'awaiting_approval'
RETURNING id
`

type MarkApprovalGateExpiredParams struct {
	ID            pgtype.UUID
	RunID         pgtype.UUID
	AwaitingSince pgtype.Timestamptz
}
...
func (q *Queries) MarkApprovalGateExpired(ctx context.Context, arg MarkApprovalGateExpiredParams) (pgtype.UUID, error) {
	row := q.db.QueryRow(ctx, markApprovalGateExpired, arg.ID, arg.RunID, arg.AwaitingSince)
	var id pgtype.UUID
	err := row.Scan(&id)
	return id, err
}
