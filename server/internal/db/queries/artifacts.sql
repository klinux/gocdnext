...
-- Inserts a row in `pending` status ahead of the agent's upload. If a
-- previous RequestArtifactUpload attempt already created a pending row for
-- the same (job_run_id, path), reissue that row with an updated retention
-- timestamp so a lost gRPC response or mid-batch signing failure is
-- recoverable. A ready active row is never reopened.
...
ON CONFLICT (job_run_id, path) WHERE deleted_at IS NULL
...
-- match the agent's ArtifactRef. Bumps status + records the server-observed
...
-- name: RetireArtifactsByJobRun :exec
