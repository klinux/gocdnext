...
func TestBuildAssignment_InjectsSecretsIntoEnvAndMasks(t *testing.T) {
	...
	got, _, err := scheduler.BuildAssignment(run, job, nil, secrets, nil, store.ResolvedProfile{}, nil, nil, nil, nil, "", nil)
	if err != nil {
...
func TestBuildAssignment_MasksOptInOutputsBypassesEightCharHeuristic(t *testing.T) {
	...
	got, _, err := scheduler.BuildAssignment(run, job, nil, nil, nil, store.ResolvedProfile{}, nil, needs, nil, nil, "", nil)
	if err != nil {
		t.Fatalf("BuildAssignment: %v", err)
	}
	masks := map[string]bool{}
...
func TestBuildAssignment_MergesProfileEnvAndMasks(t *testing.T) {
	...
	got, _, err := scheduler.BuildAssignment(run, job, nil, nil, nil, store.ResolvedProfile{Env: profileEnv, SecretValues: profileMasks}, nil, nil, nil, nil, "", nil)
	if err != nil {
		t.Fatalf("BuildAssignment: %v", err)
	}
...
func TestBuildAssignment_PropagatesProfileNodeSelectorAndTolerations(t *testing.T) {
	...
	got, _, err := scheduler.BuildAssignment(run, job, nil, nil, nil, resolved, nil, nil, nil, nil, "", nil)
	if err != nil {
		t.Fatalf("BuildAssignment: %v", err)
	}
...
func TestBuildAssignment_MissingSecretIsError(t *testing.T) {
	...
	if _, _, err := scheduler.BuildAssignment(run, job, nil, map[string]string{}, nil, store.ResolvedProfile{}, nil, nil, nil, nil, "", nil); err == nil {
...
func TestBuildAssignment_MapsTasksAndCheckouts(t *testing.T) {
	...
	got, _, err := scheduler.BuildAssignment(run, job, materials, nil, nil, store.ResolvedProfile{}, nil, nil, nil, nil, "", nil)
	if err != nil {
		t.Fatalf("BuildAssignment: %v", err)
	}
...
func TestBuildAssignment_RejectsUnresolvedRefBeforeDispatch(t *testing.T) {
	...
	_, _, err := scheduler.BuildAssignment(run, job, nil,
		map[string]string{"DOCKER_USERNAME": "deploybot"}, nil, store.ResolvedProfile{}, nil, nil, nil, nil, "", nil)
	if err == nil {
...
func TestBuildAssignment_ResolvesNeedsOutputsInEnv(t *testing.T) {
	...
	got, _, err := scheduler.BuildAssignment(run, job, nil, nil, nil, store.ResolvedProfile{}, nil, needsOutputs, nil, nil, "", nil)
	if err != nil {
		t.Fatalf("BuildAssignment: %v", err)
	}
...
func TestBuildAssignment_NeedsRefErrorsWrapSentinel(t *testing.T) {
	...
	_, _, err := scheduler.BuildAssignment(run, job, nil, nil, nil, store.ResolvedProfile{}, nil, needsOutputs, nil, nil, "", nil)
	if err == nil {
...
func TestBuildAssignment_SubstitutesCIVarsAndShellRefs(t *testing.T) {
	...
	got, _, err := scheduler.BuildAssignment(run, job, nil,
		...
		nil, store.ResolvedProfile{}, nil, nil, nil, nil, "", nil)
	if err != nil {
		t.Fatalf("BuildAssignment: %v", err)
	}
	plug := got.Tasks[0].GetPlugin()
...
func TestBuildAssignment_CloneTokenRewritesURLAndMasks(t *testing.T) {
	...
	got, _, err := scheduler.BuildAssignment(run, job, materials, nil, nil, store.ResolvedProfile{}, cloneTokens, nil, nil, nil, "", nil)
	if err != nil {
		t.Fatalf("BuildAssignment: %v", err)
	}
